module.exports = {
  insertUser: u => {
    ...
    if (user) {
      ...
      const newUser = {user: {...u, id: (users.length + 1).toString()}, token: u.email}
      users = [...users, newUser];
      return newUser;
    }
  },
  ...
  removePlayer: (playerId, userId) => {
    const player = players.find(player => player.id === playerId && player.created_by === userId)
    if (player) {
  ...
  insertPlayer: ({ first_name, last_name, rating, handedness, created_by }) => {
    ...
    const newPlayer = { first_name, last_name, rating, handedness, created_by, id: (players.length + 1).toString() };
